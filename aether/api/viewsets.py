...
class ForumPostViewSet(ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        qs = ForumPost.objects\
            .select_related('user', 'user__profile')\
            .prefetch_related('edits')\
            .filter(deleted=False)\
            .defer('user__first_name', 'user__last_name', 'user__email', 'user__last_login', 'user__password',
        ...
        if user and user.is_authenticated:
            subq = User.objects\
                ...
                .values('pk')
            qs = qs.annotate(can_read=Exists(subq))\
                .filter(Q(thread__board__read_perm=None) | Q(can_read=True))
        else:
